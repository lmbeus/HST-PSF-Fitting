...
binary_residual_error_array = np.load('residual_error_array.npy')
...
outputs = []
for i in range(len(binary_residual_error_array)):
	residual_error = binary_residual_error_array[i]
...
for i in top_three:
	...
	dist_outputs.append((residual_error, center, secondary, best_primary, flux_primary, best_secondary, flux_secondary, flux_sum))
...
for itr,output in enumerate(dist_outputs):
	separations = []
	angles = []
	for n in range(size):
		cx=int((array_of_PSFs[0].shape[1]-1)/2)
	...
	if itr == 0:
		separations_1 = separations
		angles_1 = angles
	elif itr == 1:
		separations_2 = separations
		angles_2 = angles
	elif itr == 2:
		separations_3 = separations
		angles_3 = angles

#separations
plt.figure()
plt.hist(separations_1)
plt.title('Model #'+ str(a) + '\n Primary Flux: %.2f - Primary Coorinates: (%.2f,%.2f)\n Error: %.2f\n Separation: %.2f'
	%(flux_primary_array[a], center_array[a][1], center_array[a][0], binary_residual_error_array[a], separations_array[a]))
plt.savefig('separations_' + str(a) + 'plot.png')
plt.close()
#angles
plt.figure()
plt.hist(angles_1)
plt.title('Model #'+ str(a) + '\n Primary Flux: %.2f - Primary Coorinates: (%.2f,%.2f)\n Error:%.2f\n Angle: %.2f'
	%(flux_primary_array[a], center_array[a][1], center_array[a][0], binary_residual_error_array[a], angles_array[a]))
plt.savefig('angles_' + str(a) + 'plot.png')
plt.close()

#separations
plt.figure()
plt.hist(separations_2)
plt.title('Model #'+ str(b) + '\n Primary Flux: %.2f - Primary Coorinates: (%.2f,%.2f)\n Error:%.2f\n Separation: %.2f'
	%(flux_primary_array[b], center_array[b][1], center_array[b][0], binary_residual_error_array[b], separations_array[b]))
plt.savefig('separations_' + str(b) + 'plot.png')
plt.close()
#angles
plt.figure()
plt.hist(angles_2)
plt.title('Model #'+ str(b) + '\n Primary Flux: %.2f - Primary Coorinates: (%.2f,%.2f)\n Error:%.2f\n Angle: %.2f'
	%(flux_primary_array[b], center_array[b][1], center_array[b][0], binary_residual_error_array[b], angles_array[b]))
plt.savefig('angles_' + str(b) + 'plot.png')
plt.close()

#separations
plt.figure()
plt.hist(separations_3)
plt.title('Model #'+ str(c) + '\n Primary Flux: %.2f - Primary Coorinates: (%.2f,%.2f)\n Error:%.2f\n Separation: %.2f'
	%(flux_primary_array[c], center_array[c][1], center_array[c][0], binary_residual_error_array[c], separations_array[c]))
plt.savefig('separations_' + str(c) + 'plot.png')
plt.close()
#angles
plt.figure()
plt.hist(angles_3)
plt.title('Model #'+ str(c) + '\n Primary Flux: %.2f - Primary Coorinates: (%.2f,%.2f)\n Error:%.2f\n Angle: %.2f'
	%(flux_primary_array[c], center_array[c][1], center_array[c][0], binary_residual_error_array[c], angles_array[c]))
plt.savefig('angles_' + str(c) + 'plot.png')
plt.close()

#Plot distribution of fluxes for single fit
fluxes = np.load('flux_distributions.npy')
plt.figure()
plt.title('Single Flux: %.2f - Coordinates: ' + str(flux_array[-1]))
plt.hist(fluxes, 10)
...
plt.title('Model #'+ str(a) + '\n Primary Flux: %.2f - Primary Coorinates: (%.2f,%.2f)\n Error:%.2f'
	%(flux_primary_array[a], center_array[a][1], center_array[a][0], binary_residual_error_array[a]))
plt.savefig('primary_fluxes_' + str(a) + '_plot.png')
plt.close()
#secondary
plt.figure()
plt.hist(secondary_fluxes1, 10)
plt.title('Model #'+ str(a) + '\n Secondary Flux: %.2f - Secondary Coorinates: (%.2f,%.2f)\n Error:%.2f'
	%(flux_secondary_array[a], secondary_array[a][1], secondary_array[a][0], binary_residual_error_array[a]))
...
#ADD LABELS AND TITLES
plt.title('Model #'+ str(b) + '\n Primary Flux: %.2f - Primary Coorinates: (%.2f,%.2f)\n Error:%.2f'
	%(flux_primary_array[b], center_array[b][1], center_array[b][0], binary_residual_error_array[b]))
plt.savefig('primary_fluxes_' + str(b) + '_plot.png')
plt.close()
plt.figure()
plt.hist(secondary_fluxes2, 10)
plt.title('Model #'+ str(b) + '\n Secondary Flux: %.2f - Secondary Coorinates: (%.2f,%.2f)\n Error:%.2f'
	%(flux_secondary_array[b], secondary_array[b][1], secondary_array[b][0], binary_residual_error_array[b]))
...
#ADD LABELS AND TITLES
plt.title('Model #'+ str(c) + '\n Primary Flux: %.2f - Primary Coorinates: (%.2f,%.2f)\n Error:%.2f'
	%(flux_primary_array[c], center_array[c][1], center_array[c][0], binary_residual_error_array[c]))
plt.savefig('primary_fluxes_' + str(c) + '_plot.png')
plt.close()
plt.figure()
plt.hist(secondary_fluxes3, 10)
plt.title('Model #'+ str(c) + '\n Secondary Flux: %.2f - Secondary Coorinates: (%.2f,%.2f)\n Error:%.2f'
	%(flux_secondary_array[c], secondary_array[c][1], secondary_array[c][0], binary_residual_error_array[c]))
