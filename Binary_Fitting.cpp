	...
	xt::xarray<double> flux_sum_array = xt::zeros<double>({coordinates.shape()[0]});
		...
		xt::xarray<int> w2 = w_array(2);
    		...
    		while (abs(comp - flux) > .000001 && iteration < 10) {
    			comp = flux;
    			xt::xarray<int> best_PSFs =  BinaryPSF(array_of_PSFs, secondary_coordinates, image_psf, flux, w);
    			best_primary = best_PSFs(0);
    			best_secondary = best_PSFs(1);
			
    			xt::xarray<double> best_PSFs_ref = BinaryRelativeFlux(array_of_PSFs, secondary_coordinates, image_psf, flux, w, best_primary, best_secondary);
